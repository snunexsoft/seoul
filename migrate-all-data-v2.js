...
async function migrateAllData() {
  try {
    ...
    // 외래 키 제약 때문에 먼저 posts와 categories 삭제
    ...
    for (const board of boards) {
      await pgPool.query(
        'INSERT INTO boards (id, name, slug, description, created_at) VALUES ($1, $2, $3, $4, $5)',
        [
          board.id,
          board.name,
          board.slug,
    ...
    for (const post of posts) {
      await pgPool.query(
        'INSERT INTO posts (id, title, content, board_id, category_id, created_at, view_count) VALUES ($1, $2, $3, $4, $5, $6, $7)',
        [
          post.id,
          post.title,
          post.content,
    ...
    for (const item of history) {
      await pgPool.query(
        'INSERT INTO history (id, year, month, date_text, title, description, sort_order, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
        [
          item.id,
          item.year,
          item.month,
    ...
    for (const post of linkPosts) {
      await pgPool.query(
        'INSERT INTO link_posts (id, title, content, link_url, image_url, main_category, sub_category, status, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
        [
          post.id,
          post.title,
          post.content || post.title, // content가 없으면 title을 사용
          post.url || post.link_url,
          post.image_url,
          post.main_category || '기타', // 없으면 기본값
          post.sub_category || '일반', // 없으면 기본값
    ...
    for (const data of energyData) {
      await pgPool.query(
        'INSERT INTO energy_data (building_name, year, month, electricity, gas, water, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7)',
        [
          data.building_name,
          data.year,
          data.month,
    ...
    for (const data of solarData) {
      ...
      await pgPool.query(
        'INSERT INTO solar_data (location, building_name, year, month, generation, created_at) VALUES ($1, $2, $3, $4, $5, $6)',
        [
          location,
          data.building_name,
          data.year,
    ...
    for (const file of files) {
      await pgPool.query(
        'INSERT INTO files (id, title, filename, created_at) VALUES ($1, $2, $3, $4)',
        [
          file.id,
          file.title,
          file.filename,
    ...
    for (const page of pages) {
      await pgPool.query(
        'INSERT INTO pages (id, title, slug, content, created_at) VALUES ($1, $2, $3, $4, $5)',
        [
          page.id,
