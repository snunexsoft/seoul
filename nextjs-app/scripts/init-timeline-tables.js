...
try {
  ...
  console.log('현재 아이콘 링크 개수:', iconCount.count);
  ...
  console.log('현재 타임라인 배경 이미지 개수:', bgCount.count);
  ...
  if (bgCount.count === 0) {
    ...
    const insertManyBg = db.transaction((images) => {
      for (let i = 0; i < images.length; i++) {
        insertBg.run(`/img/${images[i]}`, i + 1);
      }
    });

    insertManyBg(backgroundImages);
    console.log('✅ 기본 타임라인 배경 이미지 추가됨');
  }

  // 테이블 확인
  const tables = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name IN ('timeline_backgrounds', 'icon_links')").all();
