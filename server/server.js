...
// ========== link_posts API (공개) ==========
app.get('/api/public/link-posts', (req, res) => {
  const { main_category, sub_category, page = 1, limit = 10 } = req.query;
  let query = "SELECT * FROM link_posts WHERE status = 'published'";
  const params = [];
  ...
  db.all(query, params, (err, rows) => {
    ...
    res.json({
      ...
      totalPages: Math.ceil(rows.length / parseInt(limit)),
      totalPosts: rows.length
    });
  });
});

app.get('/api/public/link-posts/:id', (req, res) => {
  db.get('SELECT * FROM link_posts WHERE id = ?', [req.params.id], (err, row) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    if (!row) {
...
// link_posts 관리자 CRUD
app.get('/api/link-posts', authMiddleware, (req, res) => {
  db.all('SELECT * FROM link_posts ORDER BY created_at DESC', (err, rows) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    res.json(rows);
...
app.post('/api/link-posts', authMiddleware, (req, res) => {
  const { title, content, link_url, image_url, main_category, sub_category, status } = req.body;
  db.run(
    'INSERT INTO link_posts (title, content, link_url, image_url, main_category, sub_category, status) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [title, content, link_url, image_url, main_category, sub_category, status || 'published'],
    function(err) {
      if (err) {
        return res.status(500).json({ error: 'Database error' });
      }
...
app.put('/api/link-posts/:id', authMiddleware, (req, res) => {
  const { title, content, link_url, image_url, main_category, sub_category, status } = req.body;
  db.run(
    'UPDATE link_posts SET title = ?, content = ?, link_url = ?, image_url = ?, main_category = ?, sub_category = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
    [title, content, link_url, image_url, main_category, sub_category, status, req.params.id],
    (err) => {
      if (err) {
        return res.status(500).json({ error: 'Database error' });
      }
...
app.delete('/api/link-posts/:id', authMiddleware, (req, res) => {
  db.run('DELETE FROM link_posts WHERE id = ?', [req.params.id], (err) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    res.json({ success: true });
...
app.get('/api/public/history', (req, res) => {
  ...
  query += ' ORDER BY year ASC, sort_order ASC';
...
app.get('/api/public/history/years', (req, res) => {
  db.all('SELECT DISTINCT year FROM history ORDER BY year ASC', (err, rows) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
...
app.get('/api/history', authMiddleware, (req, res) => {
  db.all('SELECT * FROM history ORDER BY year DESC, sort_order ASC', (err, rows) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    res.json(rows);
...
app.post('/api/history', authMiddleware, (req, res) => {
  const { year, month, day, date_text, title, description, sort_order } = req.body;
  db.run(
    'INSERT INTO history (year, month, day, date_text, title, description, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [year, month, day, date_text, title, description, sort_order || 0],
    function(err) {
      if (err) {
        return res.status(500).json({ error: 'Database error' });
      }
...
app.put('/api/history/:id', authMiddleware, (req, res) => {
  const { year, month, day, date_text, title, description, sort_order } = req.body;
  db.run(
    'UPDATE history SET year = ?, month = ?, day = ?, date_text = ?, title = ?, description = ?, sort_order = ? WHERE id = ?',
    [year, month, day, date_text, title, description, sort_order, req.params.id],
    (err) => {
      if (err) {
        return res.status(500).json({ error: 'Database error' });
      }
