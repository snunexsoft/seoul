...
export default function EnergyData() {
  ...
  const fetchEnergyData = async () => {
    try {
      ...
      const response = await axios.get(`/api/energy-data?${params}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingData) {
        await axios.put(`/api/energy-data/${editingData.id}`, formData);
        toast.success('데이터가 수정되었습니다');
      } else {
        await axios.post('/api/energy-data', formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`/api/energy-data/${id}`);
    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (const row of jsonData) {
          await axios.post('/api/energy-data', {
